...
\noindent where $\left[ \mathbf{A} \right]^{k}$ is the matrix of coefficients, $\mathbf{h}^{k}$ is the vector of heads, and $\mathbf{b}^{k}$ is the vector of constant terms. Under the Newton-Raphson scheme the Jacobian $\left[ \mathbf{J} \right]^{k}$ takes the place of $\left[ \mathbf{A} \right]^{k}$. The residual is
...
Cells are indexed by $n$ and $m$, so $C_{n,m}$ is the conductance between two cells and $\eta_{n}$ is the list of cells connected to cell $n$. A superscript $k$ denotes the current time step and $k-1$ the previous one.
...
The chapters that follow add symbols where they describe a \mf{} formulation directly, and those keep the names used in the \mf{} documentation \citep{langevin2017}: $SC1$ for the primary storage coefficient, $S_s$ and $S_y$ for specific storage and specific yield, and $S_F$ for the fractional saturation of a cell.
